...
contract ZKEmailSignerTest is BaseTest {
    ...
    ZKEmailSigner public zkEmailSigner = ZKEmailSigner(address(bytes20(keccak256("ZKEmailSigner"))));
    ...
    function _deployZKEmailSigner() internal {
        ...
        IthacaAccountRecoveryCommandHandler tempCommandHandler =
            new IthacaAccountRecoveryCommandHandler(address(zkEmailSigner));
    ...
    function _setupTestAccount() internal {
        ...
        compromisedKey.k.isSuperAdmin = true;

        // Create new replacement key
        newKey = _randomSecp256k1PassKey();
        newKey.k.isSuperAdmin = true;
    ...
    function _configureGuardians() internal {
        ...
        zkEmailSigner.configureRecovery(
            guardians, THRESHOLD, uint128(RECOVERY_DELAY), uint128(RECOVERY_EXPIRY)
        );
    }

    function _calculateRecoveryDataHash() internal {
        // Calculate recovery data hash the same way as IthacaAccountRecoveryCommandHandler
        ...
        uint256 nonce = IIthacaAccount(testAccount.eoa).getNonce(69);
        recoveryDataHash = IIthacaAccount(testAccount.eoa).computeDigest(calls, nonce);
    }
    ...
    function _verifyRecoveryCompleted() internal {
        // two step assertion first check if the key is revoked
        vm.expectRevert(bytes4(keccak256("KeyDoesNotExist()")));
        testAccount.d.getKey(_hash(compromisedKey.k));
        // then check if the new key is authorized
        assertEq(testAccount.d.getKey(_hash(newKey.k)).publicKey, newKey.k.publicKey);
    }

    function _generateAcceptanceEmailAuth(uint256 guardianIndex)
        internal
        returns (EmailAuthMsg memory)
    {
        string memory command =
            string.concat("Accept guardian request for ", _addressToString(testAccount.eoa));

        bytes32 nullifier = keccak256(abi.encode("acceptance", guardianIndex, nullifierCount++));

        EmailProof memory proof =
            _generateMockEmailProof(command, nullifier, accountSalts[guardianIndex]);
    ...
    function _generateRecoveryEmailAuth(uint256 guardianIndex)
        internal
        returns (EmailAuthMsg memory)
    {
    ...
    function _generateMockEmailProof(string memory command, bytes32 nullifier, bytes32 accountSalt)
        internal
    ...
    function testIsValidSignatureWithKeyHash() public {
        ...
        assertEq(result, bytes4(0x8afc93b4)); // _MAGIC_VALUE
    }
    ...
    function testIsValidSignatureWithKeyHashInvalidDigest() public {
        ...
        bytes4 result = zkEmailSigner.isValidSignatureWithKeyHash(
            0xdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeefdeadbeef, bytes32(0), ""
        );
    ...
    function testIsValidSignatureWithKeyHashInsufficientGuardians() public {
        ...
        EmailAuthMsg memory emailAuthMsg = _generateRecoveryEmailAuth(0);
        zkEmailSigner.handleRecovery(emailAuthMsg, 0);
